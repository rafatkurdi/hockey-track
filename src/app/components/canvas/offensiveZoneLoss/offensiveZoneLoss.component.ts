...
@Component({
  selector: 'offensiveZoneLoss-canvas',
  templateUrl: './offensiveZoneLoss.component.html',
  styleUrls: ['./offensiveZoneLoss.component.scss'],
  providers: [OffensiveZoneLossService],
})
export class OffensiveZoneLossComponent implements OnInit {
  ...
  parseEditData(data: any) {
    if (Object.keys(data).length != 0) {
      ...
      this.offensiveZoneLossId = data.offensiveZoneLossId;

      this.player1 = data.playerId;
      this.player2 = data.clearPlayerId;
  ...
  saveAndNext() {
    this.saveType.emit({
      ...
      id: this.offensiveZoneLossId,
      type: 'offensiveZoneLoss',
    });
  }
  ...
  toggleHelp() {
    if (this.contextmenu_attribute === 'clearPlayerId') {
      this.help_title = '???';
      this.help_desc = '???';
    } else if (this.contextmenu_attribute === 'type') {
      this.help_title = '???';
      this.help_desc = '???';
    } else if (this.contextmenu_attribute === 'playerId') {
      this.help_title = '???';
      this.help_desc = '???';
    }
    this.showhelp = true;
  ...
  submit(type: string) {
    if (Object.keys(this.editData).length == 0) {
      if (this.validace()) {
        this.save_type = type;
        this.loading = true;
        this.createOffensiveZoneLoss();
      }
    } else {
      if (this.validace()) {
        this.save_type = type;
        this.loading = true;
        this.updateOffensiveZoneLoss();
      }
    }
  }
  ...
  validace() {
    ...
    if (this.type == '') {
      ...
      this.sendToast(
        'error',
        'Chyba!',
        'Je nutné vyplnit druh ztráty útočného pásma.'
      );
    }
    ...
    if (this.player1 == '' || this.player1 == undefined) {
      ...
      this.sendToast(
        'error',
        'Chyba!',
        'Pole útočící hráč nemůže být prázdné.'
      );
    }
  ...
  createOffensiveZoneLoss() {
    ...
    this.offensiveZoneLossService
      .createOffensiveZoneLoss(this.matchId, data)
      .subscribe(
        (data: any) => {
        ...
        (error) => {
          ...
          this.sendToast(
            'error',
            'Chyba!',
            'Během přidávání nového ztráty út. pásma došlo k chybě. Zkuste to znovu.'
          );
        }
      );
  }

  updateOffensiveZoneLoss() {
    let times = [];
    times = this.time.split(':');
    let time = Number(times[0]) * 60 + Number(times[1]);
    if (this.period == 1) {
    ...
    this.offensiveZoneLossService
      .updateOffensiveZoneLoss(this.matchId, this.offensiveZoneLossId, data)
      .subscribe(
        (data: any) => {
          this.sendCasomiraAfterEventEdit(this.time);
          ...
          this.updateSupervize(this.offensiveZoneLossId);
        },
        (error) => {
          this.logError({
            error: JSON.stringify(error),
